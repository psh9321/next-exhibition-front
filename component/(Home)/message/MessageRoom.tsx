...
const MessageRoom = ({ anotherIdKey, anotherData } : MESSAGE_ROOM) => {

    const { _id, _anotherId } = useParams();
        
    const { userInfoQuery, SetMessageQuery } = useUserInfoStore();

    const { register, handleSubmit } = useForm<MESSAGE_FORM_VALUE>({
        defaultValues : {
        ...
        <>
            <div className={messageStyles.messageRoom}>
                ...
                <form onSubmit={handleSubmit(OnSubmit)} className={messageStyles.areaBox}>
                    <TextareaAutosize {...register("message")} className={messageStyles.textarea}/>
